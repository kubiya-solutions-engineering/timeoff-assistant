    ...
    try:
        ...
        def format_messages(messages):
            ...
            for i, msg in enumerate(messages):
                ...
                message_date = msg.get('message_date', 'Unknown Date')
                
                # Calculate days difference for better context
                days_diff = ""
                try:
                    msg_datetime = datetime.fromtimestamp(float(msg['timestamp']))
                    today_datetime = datetime.now().replace(hour=0, minute=0, second=0, microsecond=0)
                    msg_date = msg_datetime.replace(hour=0, minute=0, second=0, microsecond=0)
                    diff = (today_datetime - msg_date).days
                    ...
                    if diff == 0:
                        ...
                        days_diff = f" ({{diff}} DAYS AGO)"
                    elif diff < -1:
                        days_diff = f" (IN {{abs(diff)}} DAYS)"
                except:
                    pass
                
                # Format message with enhanced date context
                lines.append(f"[MSG_ID:{{i+1}}] **{{user_name}}** on {{message_date}}{{days_diff}}: {{msg['message']}}\\nLINK: {{message_link}}\\nTIMESTAMP: {{msg['timestamp']}}\\n")
                ...
                for j, reply in enumerate(msg.get("replies", [])):
                    ...
                    reply_date = reply.get('message_date', 'Unknown Date')
                    
                    # Calculate reply date difference
                    reply_days_diff = ""
                    try:
                        reply_datetime = datetime.fromtimestamp(float(reply['timestamp']))
                        reply_msg_date = reply_datetime.replace(hour=0, minute=0, second=0, microsecond=0)
                        diff = (today_datetime - reply_msg_date).days
                        
                        if diff == 0:
                            reply_days_diff = " (TODAY)"
                        elif diff == 1:
                            reply_days_diff = " (YESTERDAY)"
                        elif diff == -1:
                            reply_days_diff = " (TOMORROW)"
                        elif diff > 1:
                            reply_days_diff = f" ({{diff}} DAYS AGO)"
                        elif diff < -1:
                            reply_days_diff = f" (IN {{abs(diff)}} DAYS)"
                    except:
                        pass
                    
                    lines.append(f"  [REPLY_ID:{{i+1}}.{{j+1}}] **{{reply_user_name}}** on {{reply_date}}{{reply_days_diff}}: {{reply['message']}}\\n  LINK: {{reply_link}}\\n  TIMESTAMP: {{reply['timestamp']}}\\n")
            
            # Add context header
            context_header = f"=== ANALYSIS CONTEXT ===\\nTODAY'S DATE: {{today_date}}\\nCURRENT TIME: {{current_time}}\\nTOTAL MESSAGES: {{len(messages)}}\\n=== END CONTEXT ===\\n\\n"
            return context_header + "\\n".join(lines)

        # Format all messages first
        messages_text = format_messages(messages)
        
        # Enhanced system prompt for time-off queries
        system_prompt = (
        ...
        if len(messages_text) > MAX_CHUNK_SIZE:
            ...
            # First element contains the context header, keep it
            context_header = message_blocks[0] if message_blocks else ""
            message_blocks = message_blocks[1:] if len(message_blocks) > 1 else []
                
            # Add the prefix back to each message
            message_blocks = ["[MSG_ID:" + block for block in message_blocks]
            
            # Create chunks of messages, ensuring each chunk has the context header
            chunks = []
            ...
            for block in message_blocks:
                if len(current_chunk) + len(block) < MAX_CHUNK_SIZE:
                    ...
                    current_chunk = context_header + block
                    
            # Add the last chunk if it's not empty and contains more than just the header
            if current_chunk.strip() != context_header.strip():
                chunks.append(current_chunk)
            ...
            # Synthesize all chunk results with a final LLM call
            synthesis_prompt = (
                "You are synthesizing analysis results from multiple chunks of Slack messages to provide a comprehensive answer to the user's time-off query.\\n\\n"
        ...
        return "The analysis timed out. This might be due to a large volume of messages. Please try with a more specific query or shorter time range."

    except Exception as e:
        logger.error(f"Error analyzing messages: {{str(e)}}")
        return f"Error during message analysis: {{str(e)}}. Please try again or contact support if the issue persists."
